"""
Informatica DAG using native Airflow 3.0.3 TaskFlow API.

This DAG demonstrates the modern TaskFlow API with proper HTTP operators
for IICS integration using the new Airflow 3.0.3 SDK structure.
"""
...
def trigger_job_with_session(**context):
    """Trigger job using session from previous tasks."""
    ...
    job_id = Variable.get('iics_job_id', default_var='placeholder-job-id')
...
@task
def monitor_job_with_session(**context):
    """Monitor job status using session from previous tasks."""
    session_data = context['task_instance'].xcom_pull(task_ids='extract_session')
...
@dag(
    dag_id='informatica_http_native_v3',
    description='Informatica Cloud integration using native Airflow 3.0.3 HTTP operators',
    schedule=None,
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=['informatica', 'http', 'native', 'airflow-3.0'],
    default_args={
        'owner': 'data_team',
...
def informatica_http_native_dag():
    """
    Native HTTP operators with TaskFlow API for Informatica Cloud.
    """

    # Task 1: Authenticate using HttpOperator
    ...
    monitor_job = monitor_job_with_session()
...
dag_instance = informatica_http_native_dag()